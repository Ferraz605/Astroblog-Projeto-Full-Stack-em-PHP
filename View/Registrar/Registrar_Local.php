...
<div class="Titulo_Principal Titulo_Principal--com-acoes">
    <div>
        <h2 class="titulo-pagina titulo-pagina--md">Novo local</h2>
        <p class="subtitulo-pagina subtitulo-pagina--md">Cadastre um local para vincular às observações.</p>
    </div>
...
<div class="formulario-observacao">
    <form method="post">
        ...
        <div class="grupo-campo">
            ...
            <input type="text" id="nomeLocal" name="nomeLocal" class="campo-texto" placeholder="Digite o nome do local..." required>
        </div>

        <div class="linha-campos">
            <div class="grupo-campo">
                <label class="rotulo-campo" for="cidade">Cidade</label>
                <input type="text" id="cidade" name="cidade" class="campo-texto" placeholder="Digite a cidade..." required>
            </div>
            <div class="grupo-campo">
                <label class="rotulo-campo" for="estado">Estado</label>
                <input type="text" id="estado" name="estado" class="campo-texto" placeholder="Digite o estado..." required>
            </div>
        </div>

        <div class="grupo-campo">
            <label class="rotulo-campo" for="pais">País</label>
            <input type="text" id="pais" name="pais" class="campo-texto" placeholder="Digite o país..." required>
        </div>

        <div class="grupo-campo">
            <label class="rotulo-campo" for="descricaoLocal">Descrição</label>
            <textarea id="descricaoLocal" name="descricaoLocal" class="campo-textarea" placeholder="Digite a descrição..." rows="4" required></textarea>
        </div>

        <div class="acoes-formulario">
        <button type="reset" class="botao-cancelar">Limpar</button>

        <button type="submit" class="botao-registrar">Registrar Local <?php 
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($nomeLocal && $cidade && $estado && $pais && $descricaoLocal) {
                    $sucesso = $inserir->cadastrarLocal($conexao, $nomeLocal, $cidade, $estado, $pais, $descricaoLocal);

                    if ($sucesso){
                        header('Location: Registrar_Local.php');
                        exit;
                    }
                }
            }
